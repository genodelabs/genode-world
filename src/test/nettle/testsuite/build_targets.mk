...
TESTS += ed25519-test
TESTS += ed448-test
# TESTS += eddsa-compress-test   <-- build error, hogweed needed ?, deactivated
...
# TESTS += rsa-encrypt-test      <-- build error, ???, deactivated
# TESTS += rsa-keygen-test       <-- build error, ???, deactivated
# TESTS += rsa-pss-sign-tr-test  <-- build error, ???, deactivated
# TESTS += rsa-sec-decrypt-test  <-- runtime error: Error: LD: jump slot relocation failed for symbol: '__gmpn_addaddmul_1msb0'
# TESTS += rsa-sign-tr-test      <-- build error, ???, deactivated
# TESTS += rsa-test              <-- build error, ???, deactivated
TESTS += salsa20-test
TESTS += serpent-test
